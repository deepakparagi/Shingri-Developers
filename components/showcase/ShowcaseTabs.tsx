...
export default function ShowcaseTabs() {
    ...
    return (
        <section className="py-24 px-4 md:px-8 lg:px-24 bg-white min-h-[80vh] flex flex-col justify-center">
                ...
                <div className="inline-flex bg-gray-100 p-1 rounded-full relative">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`relative px-6 py-2 rounded-full text-sm font-medium transition-colors z-10 ${activeTab === tab.id ? "text-white" : "text-gray-500 hover:text-black"
                                }`}
                        >
                            {activeTab === tab.id && (
                                <motion.div
            ...
            {/* Content Display */}
            <div className="relative h-[400px] md:h-[500px] w-full max-w-5xl mx-auto rounded-3xl overflow-hidden shadow-2xl border border-gray-100">
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        className={`absolute inset-0 flex flex-col md:flex-row items-center justify-between p-12 bg-gradient-to-br ${content[activeTab as keyof typeof content].gradient}`}
                    >
                        <div className="md:w-1/2 space-y-6">
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className="px-6 py-3 bg-black text-white rounded-full font-medium mt-4"
                            >
                                Learn more
                            </motion.button>
                        </div>
                        ...
                        <div className="md:w-1/2 h-full flex items-center justify-center relative">
                            ...
                            <motion.div
                                ...
                                className={`w-64 h-64 rounded-3xl shadow-lg ${tabs.find(t => t.id === activeTab)?.color}`}
                            />
                        </div>
                    </motion.div>
                </AnimatePresence>
